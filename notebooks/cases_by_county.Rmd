...
cases_new <- rgdal::readOGR('https://opendata.arcgis.com/datasets/fbae539746324ca69ff34f086286845b_0.geojson', verbose = F)
```
...
```{r}
## append pop counts to `daily_cases_cty` from `cases_new`
daily_cases_cty$pop <- cases_new$pop[match(daily_cases_cty$attribute, cases_new$attribute)]
```
...
```{r}
fnx <- paste0(paste('daily_cases_by_county', Sys.Date()-1, sep = '_'), '.RData')
fnx <- file.path('data', 'legacy', fnx)
out <- daily_cases_cty[,-ncol(daily_cases_cty)]  # leave `cluster` column out
save(out, file = fnx)
```
